...
describe do
  ...
  def radius(lat, lon, radius)
    radius = radius * 0.004454741     #1km =~ 0.004454741
    rez = []
    Issue.all.each do |issue| 
        rez.push(issue) if in_circle(lat, lon, radius , issue.lat, issue.lon)
    end
    return rez
  end 
  ...
  expect {
    ...
    issue9 = create :issue, name: "Lacul 3", lat: 46.76741063833104, lon: 23.591589331626892
    issue10 = create :issue, name: "Lac ghiorgheni", lat: 46.76741063833104, lon: 23.591589331626892
    expect(issue1.lat).to eq 46.77127690853819
  }.to change { Issue.count}.by(10)
  assert_equal(radius(46.77064449059386, 23.59665870666504, 1).count,2)
  assert_equal(radius(46.77064449059386, 23.59665870666504, 0).count,0)
  assert_equal(radius(46.77064449059386, 23.59665870666504, 10).count,10)
  end
end
